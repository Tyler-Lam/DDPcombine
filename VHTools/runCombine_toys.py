...
dir_out = "combine_{}".format(options.date) #output directory
dir_in = "{CMSSW_BASE}/src/datacards_{date}".format(CMSSW_BASE = os.environ["CMSSW_BASE"], date = options.date) #path to datacards
        ...
        for ct in ctaus:
            for v in Vs:
                for l in ['ELE', 'MU']:
                    ...
                    cmd = "combineCards.py "
                    files = glob.glob("{}/datacard_{}_bin*.txt".format(dir_in, name))
                    for f in files:
                        cmd += "{} ".format(f)
                    cmd += " > {}/datacard_{}.txt".format(dir_in, name)
                    os.system(cmd)
        ...
        for ct in ctaus:
            for v in Vs:
                for l in ['ELE', 'MU']:
                    ...
                    cmd = "combineCards.py "
                    for y in years:
                        cmd += "{}/datacard_{}_{}.txt ".format(dir_in, name,y)
                    cmd += " > {}/datacard_{}_Run2.txt".format(dir_in, name)
                    os.system(cmd)
    ...
    for ct in ctaus:
        ...
        for v in Vs:
            ...
            for l in ['ELE', 'MU']:
                ...
                if options.grid or options.merge:
                    if (options.year, v, l, m, ct) not in gridPoints:
                        ...
                        if not options.blind and -1 in points:
                            ...
                            cmd = "combine -M HybridNew -d {i} datacard_{name}.txt {args} --grid=higgsCombine.{name}_grid.HybridNew.mH125.root -n .{name}".format(i = dir_in, name = name, args = mergeArgs.format(name=name))
                            os.system(cmd)
                        continue
                
                if options.blind:
                    asimov = "combine -d {i}/datacard_{name}.txt -M GenerateOnly -t -1 --saveToys -m 125 --bypassFrequentistFit -n .{name}_asimov".format(i = dir_in, name=name)
                    os.system(asimov)

                points = gridPoints[options.year, v, l, m, ct] if (options.year, v, l, m, ct) in gridPoints else []
                
                for q in [0.025, 0.16, 0.5, 0.84, 0.975]:
                    cmd = '''combineTool.py -M HybridNew -d {i}/datacard_{name}.txt {a} --expectedFromGrid={q} -n .{name} --rMax {rMax} {cond} --task-name {name}.quant{q}'''.format(i=dir_in, a=combineArgs.format(name=name+".quant{}".format(q), asimov=name), q=q, name=name, rMax=rMax, cond=condorArgs)
                    if q == 0.025:
                ...
                if not options.blind:
                    cmd = '''combineTool.py -M HybridNew -d {i}/datacard_{name}.txt {a} -n .{name} --rMax {rMax} {cond} --task-name {name}'''.format(i = dir_in, a=combineArgs.format(name=name), name=name, rMax=rMax, cond=condorArgs)
                    if -1 in points:
                        points = gridPoints[options.year, v, l, m, ct]
                ...
                cmd_v += "{}/datacard_{}.txt ".format(dir_in, name)
            ...
            if options.grid or options.merge:
                if (options.year, v, m, ct) not in gridPoints:
                    ...
                    for q in [0.025, 0.16, 0.5, 0.84, 0.975]:
                        ...
                        cmd = "combine -M HybridNew -d {i}/datacard_{name}.txt {args} --grid=higgsCombine.{name}_grid.HybridNew.mH125.quant{q:.3f}.root --expectedFromGrid={q} -n .{name}".format(i=dir_in, name=name, q = q, args = mergeArgs.format(name=name))
                        os.system(cmd)
                    if not options.blind and -1 in points:
                        cmd = "hadd -f higgsCombine.{}_grid.HybridNew.mH125.root higgsCombine.{}.POINT*HybridNew.mH125.root".format(name, name)
                        os.system(cmd)
            ...
            if not options.blind:
                cmd = '''combineTool.py -M HybridNew -d {i}/datacard_{name}.txt {a} --LHCmode LHC-limits --saveHybridResult -n .{name} --rMax {rMax} {cond} --task-name {name}'''.format(i= dir_in, a = combineArgs.format(name=name), name=name, rMax=rMax, cond=condorArgs)
                if -1 in points:
                    cmd += " --singlePoint {r0}:{r1}:{deltaR}".format(r0 = points[-1][0], r1 = points[-1][1], deltaR = points[-1][2])
                elif options.grid:
                    continue
            ...
            cmd_combine += "{}/datacard_{}.txt ".format(dir_in, name)
        ...
        if options.grid or options.merge:
            if (options.year, 'V', m, ct) not in gridPoints:
                ...
                for q in [0.025, 0.16, 0.5, 0.84, 0.975]:
                    ...
                    cmd = "combine -M HybridNew -d {i}/datacard_{name}.txt {args} --grid=higgsCombine.{name}_grid.HybridNew.mH125.quant{q:.3f}.root -n .{name}".format(i=dir_in, name=name, args = mergeArgs.format(name = name + ".quant{}".format(q)), q = q)
                    os.system(cmd)
                if not options.blind and -1 in points:
                    cmd = "hadd -f higgsCombine.{}_grid.HybridNew.mH125.root higgsCombine.{}.POINT*HybridNew.mH125.root".format(name, name)
                    os.system(cmd)
